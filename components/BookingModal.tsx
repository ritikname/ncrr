...
const BookingModal: React.FC<BookingModalProps> = ({ 
  ...
  const openPicker = (ref: React.RefObject<HTMLInputElement>) => {
    try { if (ref.current && 'showPicker' in ref.current) (ref.current as any).showPicker(); } catch (e) {}
  };
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="relative">
                            ...
                            <div onClick={() => openPicker(startInputRef)} className="relative w-full h-10 border rounded-lg bg-transparent flex items-center px-3 cursor-pointer">
                                ...
                                <input ref={startInputRef} type="date" min={getTodayString()} value={startDate} onChange={handleStartDateChange} className="absolute inset-0 w-full h-full opacity-0 z-20 cursor-pointer" style={{opacity: 0.01}}/>
                            </div>
                        </div>
                        <div className="relative">
                            <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Return</label>
                            <div onClick={() => openPicker(endInputRef)} className="relative w-full h-10 border rounded-lg bg-transparent flex items-center px-3 cursor-pointer">
                                <span className={`text-sm ${endDate ? 'text-gray-900' : 'text-gray-400'} font-bold flex-1 pointer-events-none`}>{endDate || 'Select Date'}</span>
                                <input ref={endInputRef} type="date" min={startDate || getTodayString()} value={endDate} onChange={handleEndDateChange} className="absolute inset-0 w-full h-full opacity-0 z-20 cursor-pointer" style={{opacity: 0.01}} />
                            </div>
                        </div>
                    </div>
