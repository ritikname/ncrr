  ...
  return (
    <div className="animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users.map((user, idx) => {
          // SAFE FIX: Handle joinedAt whether it's string, seconds, or milliseconds
          let joinDateStr = 'Just now';
          try {
             if (user.joinedAt) {
                 const timestamp = Number(user.joinedAt);
                 // If timestamp is small (seconds), multiply by 1000. If large (ms), use as is.
          ...
          return (
            <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col relative overflow-hidden group">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-1">{user.name || 'Guest'}</h3>
